...
def main():
    with open(INPUT_FILE, 'r') as fp:
        lines = [line.rstrip('\n') for line in fp]
    map_ = parse_input(lines)
    solution = solve(map_)
    print(solution)


def solve(map_: np.ndarray) -> int:
    neighbors, start_node, goal_node = build_graph(map_)
...
def build_graph(map_: np.ndarray) -> tuple[dict[Coordinate, set[Coordinate]], Coordinate, Coordinate]:
    map_height, map_width = map_.shape
    start_x = np.where(map_[0, :] == '.')[0].item()
    goal_x = np.where(map_[-1, :] == '.')[0].item()
    ...
    for y, x in it.product(range(map_height), range(map_width)):
        ...
        if map_[y, x] in '.<>^v':
            # Check up
            if y > 0 and map_[y - 1, x] in '.<>^v':
                neighbors.add((y - 1, x))
            # Check right
            if x < map_width - 1 and map_[y, x + 1] in '.<>^v':
                neighbors.add((y, x + 1))
            # Check down
            if y < map_height - 1 and map_[y + 1, x] in '.<>^v':
                neighbors.add((y + 1, x))
            # Check left
            if x > 0 and map_[y, x - 1] in '.<>^v':
                neighbors.add((y, x - 1))
        if neighbors:
            graph[(y, x)] = neighbors
